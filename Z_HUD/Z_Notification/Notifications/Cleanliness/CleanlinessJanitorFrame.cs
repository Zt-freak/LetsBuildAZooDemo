...
// Type: TinyZoo.Z_HUD.Z_Notification.Notifications.Cleanliness.CleanlinessJanitorFrame
...
{
  internal class CleanlinessJanitorFrame
  {
    ...
    public CleanlinessJanitorFrame(
      float basescale_,
      ...
      NotificationAlertStatus notifstatus)
    {
      ...
      string TextToWrite = "Hire more janitors or refine where they work by using the zoning system";
      if (!Z_DebugFlags.IsBetaVersion)
        this.button = new TextButton(this.basescale, "Hire", 40f);
    ...
    public bool UpdateCleanlinessJanitorFrame(
      Player player,
      Vector2 offset,
      float DeltaTime,
      out bool trackthis)
    {
      ...
      trackthis = !Z_DebugFlags.IsBetaVersion && this.button.UpdateTextButton(player, offset, DeltaTime);
      return (0 | (trackthis ? 1 : 0)) != 0;
    }

    public void DrawCleanlinessJanitorFrame(SpriteBatch spritebatch, Vector2 offset)
    {
      offset += this.location;
